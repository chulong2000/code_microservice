ALTER PROCEDURE [dbo].[spShift_Insert]
...
ALTER PROCEDURE [dbo].[spShift_SelectAll]
AS
BEGIN
    SET NOCOUNT ON;
...
ALTER PROCEDURE [dbo].[spShift_SelectById]
    @Id UNIQUEIDENTIFIER
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN
        SELECT shift.Id, shift.FacilityId, shift.Name, shift.StartTime, shift.EndTime, shift.IsDeleted, facility.Id, facility.Name
        FROM dbo.Shift as shift 
	    left join dbo.Facility as facility
	    on shift.FacilityId = facility.Id
	    where 1 = 0;
        RETURN;
    END
	...
	left join dbo.Facility as facility
	on shift.FacilityId = facility.Id
    WHERE (shift.FacilityId = @Id  OR shift.FacilityId IS NULL) and shift.IsDeleted = 0;
END

ALTER PROCEDURE [dbo].[spShift_SoftDelete]
    @Id UNIQUEIDENTIFIER
AS
BEGIN
    SET NOCOUNT ON;
...
ALTER PROCEDURE [dbo].[spShift_Update]
