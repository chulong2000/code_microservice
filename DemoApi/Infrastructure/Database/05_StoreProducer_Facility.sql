ALTER   PROCEDURE [dbo].[spFacility_Count_Employee]
    @Id UNIQUEIDENTIFIER
AS
BEGIN
    SET NOCOUNT ON;
    select count(*) from dbo.Facility as fac 
         inner join dbo.Employee as emp
		 on fac.Id = emp.PrimaryFacilityId
		 where fac.Id = @Id AND fac.IsDeleted = 0 and (emp.Status = 'Active' OR emp.Status = 'OnLeave')
END

ALTER   PROCEDURE [dbo].[spFacility_Count_Shift]
     @Id UNIQUEIDENTIFIER
AS
BEGIN
    SET NOCOUNT ON;
    
select count(*) from dbo.[Shift] as sh 
              left join dbo.Facility as fac 
			  on sh.FacilityId = fac.Id
              where FacilityId IS NULL OR FacilityId = @Id
              and fac.IsDeleted = 0 and sh.IsDeleted = 0
END

ALTER PROCEDURE [dbo].[spFacility_ExistsName]
    @Name      NVARCHAR(150),
...
ALTER PROCEDURE [dbo].[spFacility_Insert]
...
ALTER PROCEDURE [dbo].[spFacility_SelectById]
    @Id UNIQUEIDENTIFIER
AS
BEGIN
    SET NOCOUNT ON;
...
ALTER PROCEDURE [dbo].[spFacility_SelectList]
...
ALTER PROCEDURE [dbo].[spFacility_SoftDelete]
    @Id UNIQUEIDENTIFIER
AS
BEGIN
    SET NOCOUNT ON;
...
ALTER PROCEDURE [dbo].[spFacility_Update]
